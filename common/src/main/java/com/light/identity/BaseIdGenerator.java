...
public abstract class BaseIdGenerator implements IdGenerator {
    ...
    public BaseIdGenerator(final long siteId, final String prefix) {
        if ((siteId < 0) || (siteId > 255)) {
            throw new IllegalArgumentException("SiteId must be in [0, 255].");
        }
        this.datacenterId = 0;
        this.siteId = siteId;
        this.prefix = prefix;
     ...
     * 每个节点需要使用不同的SiteId参数，共支持0~9个集群节点。
     */
    public BaseIdGenerator(long datacenterId, final long siteId, final String prefix) {
        if ((datacenterId > 3) || (siteId < 0) || (siteId > 255)) {
            throw new IllegalArgumentException("datacenterId must be in [0, 3],siteId must be in [0, 255].");
        }

 ...
 * able to parse the locale. This class will check if a double quote has been
 * added, if so it will remove it.
 */
public class AngularCookieLocaleResolver extends CookieLocaleResolver {
    ...
    private void parseLocaleCookieIfNecessary(final HttpServletRequest request) {
        if (request.getAttribute(CookieLocaleResolver.LOCALE_REQUEST_ATTRIBUTE_NAME) == null) {
            ...
            final Cookie cookie = WebUtils.getCookie(request, this.getCookieName());
            Locale locale = null;

...
import com.light.fileserver.util.ImageUtils;
import com.light.fileserver.util.Mp3DurationUtils;
import com.light.fileserver.util.Mp3TranslateUtils;
import com.light.fileserver.util.ZipUtils;

@Service
@Transactional
public class StorageServiceImpl extends AbstractStorageService {

    @Value("${framework.fileserver.localstorage.path}")
    private String rootPath;
    @Autowired
    private StorageRecordService storageRecordService;
    @Value("${framework.fileserver.normalization.tool.ffmpeg:ffmpeg}")
    private String ffmpeg;
    @Value("${framework.fileserver.normalization.tool.silkconverter:silkconverter}")
    ...
    public File getFile(final String filePath) {
        ...
        if (filePath.matches("C20\\d+_.*")) {
            ...
            if (c == null) {
                throw NotFoundException.of("文件不存在.");
            }
    ...
    @Override
    public StorageRecord getFileStorageRecord(final Long id) {
        return this.storageRecordService.getStorageRecord(id);
    }

    @Override
    public File getFileStorageRecordsTo7Zip(final List<Long> ids, final String filePath) {
    ...
    @Override
    public StorageRecord normalize(final Long id, final boolean sync) {
        //根据ID从Core获取信息
    ...
    @Override
    public StorageRecord createCombinationImage(final List<Long> ids, final NormalizationType type) {
        final List<StorageRecord> records = this.storageRecordService.getStorageRecords(ids);
